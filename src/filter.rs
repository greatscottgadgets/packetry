...
impl<Filter: FilterOps> FilterWriter<Filter> {
    ...
    pub fn catchup<C: CaptureReaderOps>(
        ...
        for device_id in (self.next_device_id .. devices_end).iter() {
            ...
            if matches!(decision, AcceptWithChildren | FilterChildren) {
        ...
        for item_id in (self.next_item_id .. items_end).iter() {
            ...
            // Link this item to that transaction ID.
        ...
        loop {
            ...
            let decision = match &self.transaction_decision {
                ...
                None => {
                    // Does this transaction ID start the next item?
                    if let Some(item_id) =
                        self.transaction_items.get(&transaction_id)
                    {
                        // Get item or transaction decision.
                        let decision = match self.item_decisions
                            .get(item_id)
                            .copied()
                        {
                            Some(decision) => Some(decision),
            ...
            if ended {
                // This transaction is now processed.

...
#[argh(subcommand)]
enum SubcommandEnum {
    One(SubCommandOpenFileInGui),
    Two(SubCommandCliCapture),
    Three(SubCommandDevices),
}
#[derive(FromArgs, PartialEq, Debug)]
/// Opens a recording in the GUI
#[argh(subcommand, name = "open")]
struct SubCommandOpenFileInGui {
...
fn main() -> ExitCode {
    ...
    let exit_code = if let Some(subcmd) = args.sub_commands {
        match subcmd {
            ...
            SubcommandEnum::Two(captureoptions) => {

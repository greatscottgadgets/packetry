...
impl CaptureFields {
    fn handle(&mut self) -> CaptureSnapshot {
        self.reader.at(&self.snapshot)
    }
}

pub struct TreeListModel<Item, Model, RowData, ViewMode> {
    _marker: PhantomData<(Model, RowData)>,
    capture: RefCell<CaptureFields>,
    view_mode: ViewMode,
      ...
      for<'r, 's> CaptureSnapshot<'r, 's>: ItemSource<Item, ViewMode>,
{
    pub fn new(
        ...
        let (completion, item_count) = capture
            .handle()
            .item_children(None, view_mode)?;
        Ok(TreeListModel {
            _marker: PhantomData,
    ...
    fn count_to_item(
        ...
        let search_result = cap.find_child(&mut expanded, range, to_index - 1)?;
    ...
    fn count_around_offset(
        ...
        let rows_before_offset = if offset == 0 {
            ...
            self.count_to_item(cap, expanded, range, offset, node_ref)?
        };
    ...
    pub fn update(&self, model: &Model, snapshot: &Snapshot)
        -> Result<bool, Error>
    {
        self.capture.borrow_mut().snapshot = snapshot.clone();
              ...
              Rc<RefCell<T>>: NodeRcOps<Item>,
    {
        ...
        // Borrow the reader and snapshot only while we access the DB.
        let mut borrow = self.capture.borrow_mut();
        let mut cap = borrow.handle();
                ...
                if let Some(child_rc) = child_weak.upgrade() {
                    ...
                    position = match state {
                        Root(..) =>
                            ...
                            self.find_top_level_item(&mut cap, &child_rc)?,
                        Interleaved(..) => {
            ...
            if top_level_pending > 0 || second_level_pending > 0 {
                ...
                let (new_length, new_top, new_second) = match &source {
                    InterleavedSearch(expanded, full_range) => {
                        ...
                        let full_second =
                            self.count_within(&mut cap, expanded, full_range)?;
                        let full_length = full_top + full_second;
                        let new_length = full_length - old_length;
                        let new_top = full_top - old_top;
                        let new_second = new_length - new_top;
